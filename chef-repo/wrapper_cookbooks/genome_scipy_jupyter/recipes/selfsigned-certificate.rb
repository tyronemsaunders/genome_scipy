...
apps.each do |app|
  # create directory
  directory "#{node['jupyter']['directories']['ssl']}/#{app['subdomain']}.#{app['domain']}" do
    owner "root"
    group "root"
    mode "0755"
    action :create
  ...
  # create certificate
  openssl_x509 "#{node['jupyter']['directories']['ssl']}/#{app['subdomain']}.#{app['domain']}/ssl.pem" do
  ...
  openssl_dhparam "#{node['jupyter']['directories']['ssl']}/#{app['subdomain']}.#{app['domain']}/dhparam.pem" do
    key_length 2048
  end
end